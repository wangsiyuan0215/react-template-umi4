...
export default options => Component => {
    ...
    const wrapperComponent = ({ form, ...restProps }) => {
         ...
         * @returns {*}
         */
        const renderForm = ({ initialValues = {}, creatorData = {}, creatorActions = {} } = {}) => (
            <Form {...formProps}>
                {formItems.map(item => {
                    if (!item.key || !item.creator) return null;

                    const { label, key, option: itemOption, creator, ...restItemOptions } = item;
                    return (
    ...
    return Form.create({ ...restOptions, name })(wrapperComponent);
};
